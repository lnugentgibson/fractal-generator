...
angular.module('oaSylvester', ['oaUtil']).service('oaSylvester', ['oaUtil', function (oaUtil) {
  var Sylvester;
  ...
  function Vector(buffer, type, offset, length, skip) {
    ...
    if (!buffer) buffer = new ArrayBuffer(offset + byteLength);
    if (false) console.log({
      buffer: buffer,
      type: type,
      offset: offset,
    ...
    for (var i = 0; i < length; i++) {
      (function (j) {
        ...
        Object.defineProperty(_this, j, property);
      })(i);
    }var lengthProperty = {
      get: function get() {
        return length;
    ...
    Object.defineProperties(this, {
      ...
      modulus: {
        ...
        set: function set(v) {
          var r = _this.modulus;
          if (r > Sylvester.precision) _this.x(v / r);
        }
      }
    });
  ...
  Vector.prototype = {
    ...
    eql: function eql(vector) {
      ...
      do {
        if (Math.abs(this[n - 1] - vector[n - 1]) > Sylvester.precision) {
          return false;
        }
      } while (--n);
      return true;
    ...
    map: function map(fn) {
      return Vector.create(this.elements.map(function (c, i) {
        return fn(c, i + 1);
      }));
    },
    ...
    each: function each(fn) {
      this.elements.forEach(function (c, i) {
        return fn(c, i + 1);
      });
    },
    ...
    add: function add(vector) {
      ...
      return this.setElements(this.elements.map(function (x, i) {
        return x + vector[i - 1];
      }));
    },
    ...
    difference: function difference(vector) {
      if (this.length != vector.length) {
        return null;
      }
      return this.map(function (x, i) {
    ...
    multiply: function multiply(k) {
      return this.setElements(this.elements.map(function (x) {
        return x * k;
      }));
    },
    ...
    dot: function dot(vector) {
      ...
      return this.elements.reduce(function (d, c, i) {
        return d + c * vector[i];
      }, 0);
    },
    ...
    max: function max() {
      return this.elements.reduce(function (x, c) {
        return x > Math.abs(c) ? x : Math.abs(c);
      }, Math.abs(this[0]));
    },
  ...
  Matrix.prototype = {
    ...
    setSource: function setSource(source, m, n, offset, s, t) {
