...
function ShaderHelpGen() {
  function oaWebglShaderHelpers() {
    ...
    if (false)
      console.log({
        keys: constants.map(c => c.type.toUpperCase() + '_' + c.name),
        names: constants.map(c => c.name),
        strings: constants.map(c => c.data.string),
        ids: constants.map(c => c.data.id),
      });
    constants.forEach((c, i) => {
      //console.log(c);
      Object.defineProperty(this, c.type.toUpperCase() + '_' + c.name, {
        get: () => i
      });
    });
...
function SnippetGen() {
  ...
  function Snippet(name, source) {
    ...
    this.addParameter = (t, k, s, o) => {
      ...
      if (ks) {
        ...
        for (var i = 0; i + 1 < ks.length; i++) {
          ...
          if (!child)
            child = parent[ks[i].key] = ks[i + 1].type === 'o' ? {} : [];
          if (!specifiers[rk])
            specifiers[rk] = ks[i + 1].type === 'o' ? {
              parameterType: 'o',
              datatype: 'default',
              nullObject: '',
              nullProperty: '',
      ...
      s = Object.assign({}, s);
      switch (t) {
        case this.PARAMTYPE_OBJECT:
          {
            specifiers[k] = Object.assign({
              parameterType: this.PARAMTYPE_OBJECT,
              datatype: this.DATATYPE_DEFAULT,
              nullObject: '',
              nullProperty: '',
              validators: []
            }, s.filter([
        ...
        case this.PARAMTYPE_ARRAY:
          {
            specifiers[k] = Object.assign({
              parameterType: this.PARAMTYPE_ARRAY,
              datatype: this.DATATYPE_DEFAULT,
              nullArray: '',
              nullElement: '',
              delimiter: '',
        ...
        case this.PARAMTYPE_VALUE:
          {
            var constraints = {};
            switch(s.datatype) {
              case this.DATATYPE_STRING:
            ...
            specifiers[k] = Object.assign({
              parameterType: this.PARAMTYPE_VALUE,
              datatype: this.DATATYPE_DEFAULT,
              nullValue: '',
              validators: []
            }, constraints, s.filter([
              'datatype',
              'nullValue'
            ]));
            parent[ks[ks.length - 1]] = true;
    ...
    this.getParameterNames = () => specifiers.getKeys();
    this.getParameterValues = () => {
      var pvs = {};
      specifiers.getKeys().forEach(k => {
        pvs[k] = parameters[k];
      });
      return pvs;
    };
    this.setParameterValue = (k, v) => {
      ...
      switch (datatype) {
        ...
        case this.DATATYPE_FLOAT:
          ...
          if (isNaN(v))
            return;
          break;
        case this.DATATYPE_OBJECT:
          break;
        case this.DATATYPE_TEMPLATE:
          break;
        case this.DATATYPE_ARRAY:
          break;
      }
      if (validators.map(validator => validator(v)).every(valid => valid))
        parameters[k] = v;
    };
    ...
    this.getSnippetNames = () => snippets.getKeys();
  ...
  Snippet.parameterType = function parameterType(type) {
    ...
    if (!constant || constant.type !== 'paramtype')
      return null;
    return constants[type].data.id;
  };
  Snippet.parameterDataType = function parameterDataType(type) {
    var constant = constants[type];
    //console.log(constant);
    if (!constant || constant.type !== 'datatype')
      return null;
    return constants[type].data.id;
  };
  return Snippet;
...
${bodyType === 'string' ? '${P("bodyString")}' : '${S("body", "body")}'}
    ...
    parameters.apply = () => {
      this.setParameterValue('signiture.parameterList', parameters.order);
    ...
    Object.defineProperties(this, {
      ...
      bodyString: {
        ...
        set: v => {
          bodyString = v;
          this.setParameterValue('body', bodyString);
        }
      },
      bodyType: {
        get: () => bodyType,
      ...
      sname: {
        get: () => funcName + '_' + parameters.order.map(p => oaWebglShaderHelpers.glDataTypeId(p.datatype)).join('')
      }
    });
  }
...
\${D("functionDefinitions", "functionDefinitionParams")}`;
    ...
    variables.apply = () => {
      ...
      this.setParameterValue('variables', variables.order);
    };
    ...
    this.addFunction = f => {
      ...
      this.setParameterValue('functionDeclarations', functions.map(f => f.sname + 'Declaration'));
      this.setParameterValue('functionDefinitions', functions.map(f => f.sname + 'Definition'));
    };
...
angular
  ...
  .service('oaWebglHelpers', [GlHelpGen])
  .service('oaWebglShaderHelpers', [ShaderHelpGen])
  .factory('oaWebglSnippet', ['oaObject', SnippetGen])
  .factory('oaWebglFunctionSnippet', ['oaWebglSnippet', 'oaWebglShaderHelpers', FunctionSnippetGen])
  .factory('oaWebglShaderSnippet', ['oaWebglSnippet', ShaderSnippetGen])
  ...
  .factory('oaWebglProgram', ['oaWebglHelpers', 'oaWebglShaderSource', ProgramGen])
